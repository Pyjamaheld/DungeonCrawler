...
if(place_meeting(x, y + 2, obj_collision)){
    if(jump_key == 1){
        ...
        log("Jumped! Alarm set to = " + string(alarm[0]), "Test", 0);
    }
}
...
if(jump_held_key == 1 && jumped == true){    
    ...
    if(y_speed > -max_jump_speed){
        ...
        log("Incresed y_speed to = " + string(y_speed) + " / " + string(-max_jump_speed), log_class_name, 0);
        log("Curent alarm timer: " + string(alarm[0]), log_class_name, 0);
    } 
}
